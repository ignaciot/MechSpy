...
EXP_CELLS = "bronchial epithelium"
...
for filename in glob.glob("%s/*_4w_bronchial-epithelial-BEAS-2B_top_10k_genes.txt" % EXP_ROOT):
...
for chemical in list(chemicals):
    concentrations = []
    for filename in glob.glob("%s/%s_*_4w_bronchial-epithelial-BEAS-2B_top_10k_genes.txt" % (EXP_ROOT, chemical)):
        concentration = filename.split('/')[-1].split('_')[1]
        concentrations.append(concentration)
    for concentration in concentrations:
        new_time_series = dict()
        time_point_count = 0
        for filename in glob.glob("%s/%s_%s_*w_bronchial-epithelial-BEAS-2B_top_10k_genes.txt" % (EXP_ROOT, chemical, concentration)):
