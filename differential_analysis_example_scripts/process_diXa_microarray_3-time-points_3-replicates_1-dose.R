...
outputdir = args[2]
#metadata = "aristolochic_acid.tsv"
metadata = args[3]
...
treatments <- factor(c(1,1,1,2,2,2,3,3,3,4,4,4,5,5,5,6,6,6),
                     labels=c("ctrl_6h", "ctrl_24h", "ctrl_72h", 
                              "high_dose_6h", "high_dose_24h", "high_dose_72h"))
contrasts(treatments) <- cbind(Time=c(0,1,2,0,1,2),
                               high_dose_6h=c(0,0,0,1,0,0),
                               high_dose_24h=c(0,0,0,0,1,0),
                               high_dose_72h=c(0,0,0,0,0,1))
...
# contrast control 6hr with treatment 6hr
...
write.table(diff_exp_brief, 
            file=paste(outputdir, 
                       paste(chem_long_name, 
                             paste(chem_concentr, 
                                   paste(chem_concentr_unit, "6h", chem_class, "kidney_top_10k_genes.txt", sep="_"), 
                                   sep="-"), 
                             sep="_"), 
                       sep="/"), 
...
cont.matrix <- cbind(ctrl8=c(0,1,0,0,0,0),treat8=c(0,0,0,0,1,0))
fit2 <- contrasts.fit(fit, cont.matrix)
fit2 <- eBayes(fit2)
i <- grep("AFFX",featureNames(eset))
options(digits=10)
...
write.table(diff_exp_brief, 
            file=paste(outputdir, 
                       paste(chem_long_name, 
                             paste(chem_concentr, 
                                   paste(chem_concentr_unit, "24h", chem_class, "kidney_top_10k_genes.txt", sep="_"), 
                                   sep="-"), 
                             sep="_"), 
                       sep="/"), 
            quote=FALSE, sep='\t', col.names = NA)
...
cont.matrix <- cbind(ctrl24=c(0,0,1,0,0,0),treat24=c(0,0,0,0,0,1))
fit2 <- contrasts.fit(fit, cont.matrix)
fit2 <- eBayes(fit2)
i <- grep("AFFX",featureNames(eset))
options(digits=10)
...
write.table(diff_exp_brief, 
            file=paste(outputdir, 
                       paste(chem_long_name, 
                             paste(chem_concentr, 
                                   paste(chem_concentr_unit, "72h", chem_class, "kidney_top_10k_genes.txt", sep="_"), 
                                   sep="-"), 
                             sep="_"), 
                       sep="/"), 
            quote=FALSE, sep='\t', col.names = NA)
