...
chemicals = set()
for filename in glob.glob("%s/*_6h_*_kidney_top_10k_genes.txt" % EXP_ROOT):
...
for chemical in list(chemicals):
    concentrations = []
    for filename in glob.glob("%s/%s_*_6h_*_kidney_top_10k_genes.txt" % (EXP_ROOT, chemical)):
        concentration = filename.split('/')[-1].split('_')[1]
        concentrations.append(concentration)
    for concentration in concentrations:
        time_point_count = 0
        new_time_series = dict()
        for filename in glob.glob("%s/%s_%s_*h_*_kidney_top_10k_genes.txt" % (EXP_ROOT, chemical, concentration)):
        ...
        if time_point_count > 1:    # We want at the very least two time points with enough genes
            EXPERIMENTS["%s (%s) diXa-003 kidney [%s]" % (chemical, concentration, EXP_CELLS)] = new_time_series
            EXP_LABELS.append('?')
        else:
